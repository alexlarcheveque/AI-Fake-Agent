...
interface BulkLeadFormProps {
  onLeadsCreated: (leads: Lead[]) => void;
}
...
const BulkLeadForm: React.FC<BulkLeadFormProps> = ({ onLeadsCreated }) => {
  ...
  const [uploadResult, setUploadResult] = useState<{ success: boolean; message: string; created: number; failed: number } | null>(null);
  ...
  const validateHeaders = (headers: string[]): boolean => {
    ...
    const hasPhoneNumberField = headers.some(
      h => h.toLowerCase() === "phone number" || h.toLowerCase() === "phonenumber" || h.toLowerCase() === "phone"
    );
    ...
    if (missingFields.length > 0) {
      setValidationErrors([`Missing required headers: ${missingFields.join(", ")}`]);
      return false;
    }
  ...
  const readCSVFile = (file: File) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        setValidationErrors(["Invalid CSV format. Please check your file and try again."]);
        setPreview(null);
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Create FormData with both file and AI feature configurations
      ...
      // Use the existing API function but pass the formData with AI features
      const result = await leadApi.importLeadsFromCSV(selectedFile, aiFeatures);
      ...
      if (Array.isArray(errorDetails) && errorDetails.length > 0) {
        ...
        setValidationErrors([error.response?.data?.error || "Failed to upload leads"]);
      }
    } finally {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      {uploadResult && (
        <div className={`p-4 ${uploadResult.success ? "bg-green-50 border-l-4 border-green-400 text-green-700" : "bg-yellow-50 border-l-4 border-yellow-400 text-yellow-700"}`}>
      ...
      <div className="bg-white rounded-lg shadow p-6">
        ...
        <div className="mb-4">
          <p className="text-gray-700 mb-2">Upload a CSV file with the following standardized columns:</p>
            ...
            <table className="min-w-full border border-gray-300">
              ...
              <tbody>
                <tr>
                  ...
                  <td className="border border-gray-300 px-4 py-2 font-semibold text-red-600">Yes</td>
                ...
                <tr>
                  ...
                  <td className="border border-gray-300 px-4 py-2">No</td>
                  <td className="border border-gray-300 px-4 py-2">
                    Lead status: new, contacted, qualified, or lost<br/>
                    <span className="text-xs text-gray-500">Defaults to "new" if not specified</span>
                  </td>
                </tr>
              </tbody>
            </table>
          ...
          <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-md">
            <h3 className="text-sm font-semibold text-blue-800 mb-1">💡 Tips for successful import:</h3>
            <ul className="list-disc text-xs text-blue-700 ml-5 space-y-1">
              <li>Keep the column headers exactly as shown in the template for best results</li>
        ...
        >
          ...
          <svg className="mx-auto h-12 w-12 text-gray-400" stroke="currentColor" fill="none" viewBox="0 0 48 48" aria-hidden="true">
            <path d="M28 8H12a4 4 0 00-4 4v20m32-12v8m0 0v8a4 4 0 01-4 4h-8m-12 0H8m12 0a4 4 0 010-8m0 0v-8m0 0H8m12 9.5V20m0 0V8m20 12v8" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
          </svg>
        ...
        {preview && (
          <div className="mt-6">
            <h3 className="text-lg font-semibold mb-2">Preview ({preview.rows.length} of {preview.totalRows} rows)</h3>
            <div className="overflow-x-auto border border-gray-200 rounded-md max-h-[300px] overflow-y-auto">
              <table className="min-w-full">
                <thead className="bg-gray-100 sticky top-0">
                  <tr>
                    {preview.headers.map((header, index) => (
                      <th key={index} className="border-b border-gray-300 px-4 py-2 text-left whitespace-nowrap">{header}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {preview.rows.map((row, rowIndex) => (
                    <tr key={rowIndex} className={rowIndex % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                      {row.map((cell, cellIndex) => (
                        <td key={cellIndex} className="border-b border-gray-200 px-4 py-2 truncate max-w-[200px]" title={cell}>
                          {cell}
                        </td>
                      ))}
                    </tr>
            ...
            {preview.totalRows > preview.rows.length && (
              <p className="mt-2 text-sm text-gray-500">
                Showing first {preview.rows.length} rows of {preview.totalRows} total rows
              </p>
            )}
          </div>
        )}
        ...
        <div className="mt-6 p-4 border border-blue-100 rounded-md bg-blue-50">
          <h3 className="text-md font-semibold text-blue-800 mb-3">AI Features for Imported Leads</h3>
          ...
          <div className="space-y-4">
            <div className="flex items-center">
              <input
                ...
                onChange={(e) => setAiFeatures(prev => ({ ...prev, aiAssistantEnabled: e.target.checked }))}
                className="w-4 h-4 text-blue-600 rounded border-gray-300 focus:ring-blue-500"
              />
              <label htmlFor="aiAssistantEnabled" className="ml-2 text-sm text-gray-700">
            ...
            <div className="flex items-center">
              <input
                ...
                onChange={(e) => setAiFeatures(prev => ({ ...prev, enableFollowUps: e.target.checked }))}
                className="w-4 h-4 text-blue-600 rounded border-gray-300 focus:ring-blue-500"
              />
              <label htmlFor="enableFollowUps" className="ml-2 text-sm text-gray-700">
                Enable automated follow-up messages
              </label>
            </div>
            
            <div className="mt-4">
              <label htmlFor="firstMessageTiming" className="block text-sm font-medium text-gray-700 mb-1">
              ...
              <select
                ...
                onChange={(e) => setAiFeatures(prev => ({ ...prev, firstMessageTiming: e.target.value }))}
              ...
              <p className="mt-1 text-xs text-gray-500">
                {!aiFeatures.enableFollowUps && "Enable follow-ups to set timing"}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="mt-6">
          <button
            ...
            className={`px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors 
              ${(isUploading || !selectedFile || validationErrors.length > 0) ? "opacity-50 cursor-not-allowed" : ""}`}
          >
